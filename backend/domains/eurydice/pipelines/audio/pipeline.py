...
def analyze_audio(args: dict[str, Any]) -> dict[str, Any]:
    ...
    # ── Capture quality pre-check ─────────────────────────────────────────────
    capture_quality = _capture_quality(y, sr, duration_s)
    if capture_quality["overall"] == "poor":
        reasons = []
        if not capture_quality["duration_ok"]:
            reasons.append("Recording too short — aim for at least 2 seconds")
        if not capture_quality["noise_floor_ok"]:
            reasons.append("Audio too quiet — try recording louder or closer to the mic")
        if capture_quality["clipping_detected"]:
            reasons.append("Audio clipping detected — reduce input volume")
        return {
            "mode": mode,
            "analysis_confidence": 0.0,
            "capture_quality": capture_quality,
            "performance_scores": {"timing": 0.0, "notes": 0.0, "overall": 0.0},
            "warnings": reasons,
            "_analysis_ms": round((time.perf_counter() - t0) * 1000, 1),
        }
    ...
    # Compute top-level confidence (most conservative)
    analysis_confidence = round(min(tempo_confidence, pitch_confidence), 2)
...
def _capture_quality(y: Any, sr: int, duration_s: float) -> dict[str, Any]:
    """Assess recording quality before running analysis."""
    import numpy as np

    duration_ok = duration_s >= 2.0

    # RMS-based noise floor check (threshold: ~-40 dBFS)
    rms = float(np.sqrt(np.mean(y ** 2)))
    noise_floor_ok = rms > 0.01  # ~-40 dBFS for 16-bit audio

    # Clipping detection: more than 0.1% of samples at max amplitude
    clip_threshold = 0.99
    clipping_ratio = float(np.mean(np.abs(y) > clip_threshold))
    clipping_detected = clipping_ratio > 0.001

    # Overall assessment
    if not duration_ok or not noise_floor_ok:
        overall = "poor"
